...
const TrainSimulator: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      if (!toStation) {
        // Trường hợp không nên xảy ra nếu logic đúng
        setIsRunning(false); 
        return;
      }

      if (elapsedTime >= SEGMENT_DURATION) {
        // Tàu đã đến toStation
        ...
        if (nextStationIndex >= STATIONS.length - 1) {
          ...
          // Chuẩn bị cho đoạn tiếp theo
          setSegmentStartTime(Date.now()); // Reset thời gian bắt đầu cho đoạn mới
        ...
        // Trạng thái "Đang di chuyển tới Ga X" đã được set khi bắt đầu đoạn hoặc khi đến ga trước đó
      }
    }, UPDATE_INTERVAL);
  ...
  return (
    <div className="flex flex-col items-center p-4 md:p-8 space-y-8 bg-white shadow-lg rounded-lg">
      ...
      {/* Đường ray và các ga */}
      <div className="relative w-[450px] h-16 bg-gray-200 rounded flex items-center px-[calc(TRACK_OFFSET_X-STATION_DOT_SIZE_CLASS_WIDTH/2)]">
        {/* Đường ray */}
        <div 
        ...
        >
          <TrainFront size={TRAIN_ICON_SIZE} className="text-blue-600" />
        </div>
      </div>
